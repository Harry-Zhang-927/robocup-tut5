...
bool PlaneSegmentation::initalize(ros::NodeHandle& nh)
{
  ...
  plane_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/segmentation/table_plane", 1);

  objects_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/segmentation/objects", 1);
...
{
  //ROS_INFO("This is an info message1hhh");
...
bool PlaneSegmentation::segmentCloud(CloudPtr& input, CloudPtr& plane_cloud, CloudPtr& objects_cloud)
{
  ...
  pcl::copyPointCloud(*input, *inliers, *plane_cloud);
  ...
  Q_plane_base = Eigen::Quaternionf::FromTwoVectors(n, Eigen::Vector3f::UnitZ());
  t_plane_base = n * d;
...
void PlaneSegmentation::cloudCallback(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  //#>>>>TODO: Convert the msg to the internal variabrle aw_cloud_ that holds the raw input pointcloud 
